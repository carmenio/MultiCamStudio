...
| Calibration | Controller, database, generator, service, viewer, and viewer-renderer suites; Calibration page/component/flow suites. Live viewer metadata and batch status are captured, a deterministic ten-camera production-renderer baseline freezes full HTML and geometry identities, and a 10-run production-browser baseline freezes canonical Plotly readiness and screenshot identity. | Add controlled preflight and fixed-fixture batch processing. |
