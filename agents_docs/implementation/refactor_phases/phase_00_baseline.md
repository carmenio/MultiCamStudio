...
631 tests. This includes the 21-test capture lifecycle suite, focused media-route
...
| Triangulation and 3D | Controller, service, database, API, page, viewer, and training-timeline suites cover mapping, variant resolution, diagnostics, audit, runs, results, assignments, training transitions, and pre-start/input/post-build cancellation persistence boundaries. A deterministic production-service baseline triangulates 81,000 observations into 27,000 accepted points with fixed full-result and calibration-fixture identities. A production-browser run freezes first usable WebGL render, native-input seek, playback start, and canvas screenshot identity for set 178/run 100. | Add result-not-found and remaining training/reference-media error shapes plus explicit camera-transform equivalence. |
