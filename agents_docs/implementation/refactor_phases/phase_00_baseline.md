...
| Playback-session lifecycle | Route characterization covers create, recording filtering, stream projection, join, stats, delete, and missing-after-delete | The extraction is an early Phase 1 slice, not Phase 0 completion. Cold-cache and browser-consumer evidence remain unavailable. |
| Resumable and multipart uploads | Recording controller tests cover required upload fields, set creation/reuse, task enqueueing, resumable initialization, duplicate chunk acceptance, assembly, imported signaling files, and finalize partial/all-failure behavior | Add resume-state ordering, invalid chunk indices, checksum conflicts, incomplete/missing chunks, checksum cleanup, repeated completion, filename collision, post-assembly persistence failure, and controlled upload-throughput evidence. |
| Capture sessions | Recording controller tests cover start/create/reuse, missing schema, stale active rows, owner-protected status, stop, idempotent finalize, shared ingest, partial import, and unexpected finalize errors | Add PATCH and abort matrices, no-active result, invalid transition/snapshot cases, no-success/all-import-failed finalization, concurrent finalize, and physical Stop-to-upload-init validation. |
