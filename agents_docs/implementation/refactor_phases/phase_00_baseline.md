...
| Calibration | Controller, database, generator, service, viewer, and viewer-renderer suites; Calibration page/component/flow suites. Live viewer metadata and batch status are captured, and a deterministic ten-camera production-renderer baseline freezes full HTML and geometry identities. | Add controlled preflight, fixed-fixture batch processing, and live rendered Plotly equivalence. |
