...
- Keep iOS recording stop asynchronous and bounded: sample callbacks already run on the shared capture queue, and upload retries must remain persisted and restart on foreground/network recovery. Native changes require a rebuilt dev client. See `agents_docs/troubleshooting/expo_phone_app.md`.
