...
- Detections model runs produce raw artifacts only. All later point processing belongs to the ordered, checkpointed post-processing task; keep its configuration in the dialog, preserve legacy settings/smoothing API compatibility, and never launch the legacy smoothing task from Detections. See `agents_docs/design/detections_post_processing.md`.
