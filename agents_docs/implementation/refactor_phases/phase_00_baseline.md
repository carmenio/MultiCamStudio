...
593 tests. This includes the 21-test capture lifecycle suite, focused media-route
cases, and the expanded cut validation/ffmpeg contract matrix. The complete operator suite
...
| Cameras and pairing | `test_session_cameras_controller.py`; Filming camera/page suites; camera-mobile config, signaling, selection, and pairing-presentation suites. Deterministic production-route baselines now freeze PC HMAC and EdgeRelay timed-serializer token issue/resolve latency, cache behavior, forwarded-origin URLs, and semantic identities. | Live nginx/TLS/QR plus physical iOS/Android pairing, reconnect, certificate, and transport-mismatch recovery remain unavailable. |
